...
open= cv2.cvtColor(open, cv2.COLOR_BGR2GRAY)
ret,open = cv2.threshold(open, 60, 100, cv2.THRESH_BINARY)

cv2.imshow("image",open )
ing = np.zeros((open.shape[0], open.shape[1], 3), dtype=np.uint8) 
cont, h = cv2.findContours(open, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
max=0
sel_countour=None
for coun in cont:
    if coun.shape[0]>max:
...
cv2.drawContours(crop, [sel_countour], -1, (255,255,0), 3)
