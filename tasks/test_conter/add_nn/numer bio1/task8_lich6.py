...
img = cv2.imread('/home/rodion/yuliia0/aboba/tasks/task8/numer bio1/lich6.jpg')
...
"""обрізка, квадрат"""
gray = gray[80:gray.shape[1]-80, 80:gray.shape[0]-80]

canny= cv2.Canny(gray, 220, 220 * 2)
...
for i in range(len(contours)):
    ...
    if 100*(boundRect[i][3])>boundRect[i][2] and 0*(boundRect[i][3])<boundRect[i][2]:
        lines=cv2.rectangle(gray, (x1+80, y1+80), (x2+80, y2+80), (100,10,40), 2)
...
cv2.rectangle(img, (x1+80, y1+80), (x2+80,y2+80), (100,255,40), 2)
crop = img[y1+80:80+y2, 80+x1:80+x2]
...
crop_gray = cv2.bilateralFilter(crop_gray, 11, 17, 17)
crop_gray = imutils.resize(crop_gray, width=300 )
ret, thresh = cv2.threshold(crop_gray, 90, 255, cv2.THRESH_TOZERO_INV)
