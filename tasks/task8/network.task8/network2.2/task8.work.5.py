...
original = cv2.imread('/home/rodion/yuliia0/aboba/tasks/task8/numer bio1/lich5.jpg')
...
canny_thresh = cv2.Canny(crop_gray, 270, 270 * 2)
...
for i in range(len(cont2)):
    if boundRect[i][2]>14 and boundRect[i][3]>16:
        if boundRect[i][2]<27 and boundRect[i][3]<40:
...
'''if int(boundRect[i][1])>(int(boundRect[i][1])-5) and int(boundRect[i][1])<(int(boundRect[i][1])+5):
...
number_crop=cv2.dilate(number_crop, kernel)'''
'''cv2.imshow('canny th 3',number_crop)'''
'''n,dst = cv2.threshold(number_crop, 120, 255, cv2.THRESH_BINARY_INV)
cv2.imshow('canny th dst', dst)
n1,dst1 = cv2.threshold(dst, 115, 255, cv2.THRESH_BINARY_INV)
print('picture -', i)
