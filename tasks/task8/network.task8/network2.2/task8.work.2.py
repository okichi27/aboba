...
original = cv2.imread('/home/rodion/yuliia0/aboba/tasks/task8/numer bio1/lich2.jpg')
...
crop=crop2 = original[y1-5:y2+5, x1-5:x2+5]
...
'''ramka_numer_=0
while ramka_numer_<len(sorted_numer):
    x1,y1,x2,y2,length_numer=sorted_numer[ramka_numer_]
    cv2.rectangle(crop, (x1, y1), (x2, y2), (10,10,240), 2)
    ramka_numer_+=1'''
...
while ramka_numer<len(sorted_numer):
    ...
    n,dst = cv2.threshold(number_crop, 130, 255, cv2.THRESH_BINARY)
    cv2.imshow('canny th dst', dst)
    dst=cv2.resize(dst, (28,28),)
    n1,dst1 = cv2.threshold(dst, 150, 255, cv2.THRESH_BINARY)
